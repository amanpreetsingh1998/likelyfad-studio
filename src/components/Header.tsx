...
export function Header() {
  ...
  // === LIKELYFAD CUSTOM START === (Save as Template)
  ...
  const projectsButton = (
    <button
      ...
      onClick={() => (window as any).__openProjectList?.()}
  ...
  // Admin, beside History for the same reason and on the same terms.
  //
  // No admin check here, and none is needed: proxy.ts gates "/" itself as an
  // admin path, so rendering this header is already proof the viewer passed
  // that gate. A client-side re-check would be a second answer to a question
  // the proxy has settled, arriving later and from a less trustworthy place.
  const adminButton = signedIn ? (
    <Link
      href="/admin"
      className="p-1.5 text-neutral-400 hover:text-neutral-200 hover:bg-neutral-800 rounded transition-colors"
      title="Admin dashboard — usage, users, content and the audit log"
    >
      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
  ...
  const settingsButtons = (
    <div className="flex items-center gap-0.5 ml-1 pl-1 border-l border-neutral-700/50">
      {/* The account menu used to sit here; it now anchors the far right of
          the header, past the social links. */}
      <button
